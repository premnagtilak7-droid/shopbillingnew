...
function CustomerPicker({ onChange }) {
  ...
  const search = useCallback(async value => {
    ...
    const { data, error } = await supabase.from('customers').select('id, full_name, phone, email').or(`phone.ilike.%${normalized}%,full_name.ilike.%${normalized}%`).limit(8)
  ...
  const create = async event => {
    ...
    const { data, error } = await supabase.from('customers').upsert({ full_name: newName.trim(), phone: newPhone.trim() }, { onConflict: 'phone' }).select('*').single()
...
function POS({ onInvoice, catalog, user, operator, permissions, branding, onOwnerOverride }) {
  ...
  const complete = async ({ payment, discount }) => {
    ...
    if (!saved) return
    setReceiptData({ items: cart, ...checkoutSummary, subtotal: discountedSubtotal, tax: discountedTax, total: finalTotal, discount: discountAmount, payment, cashier: operator?.name || user?.name || 'Cashier' })
    setCart([])
  ...
  const printReceipt = () => {
    ...
    printThermalReceipt({ ...printable, branding, cashier: printable.cashier || operator?.name || user?.name || 'Cashier', payment: printable.payment || 'Cash' })
  }
...
function StaffManagement({ staff, onCreateStaff, onUpdateStaff, onResetPin, onDeactivate }) {
  ...
  const submitStaff = async event => { event.preventDefault(); if (!form.full_name.trim() || !form.email.trim() || !/^\d{4}$/.test(form.access_pin) || !['owner', 'employee'].includes(form.role)) return toast.error('Full name, email, owner/employee role, and exactly 4 PIN digits are required'); setBusy(true); const ok = await onCreateStaff(form); setBusy(false); if (ok) { setForm(emptyForm); setModal(null) } }
...
function CustomerDirectory({ invoices }) {
  ...
  useEffect(() => { let mounted = true; const load = async () => { if (!supabase) return; const { data, error } = await supabase.from('customers').select('*').order('full_name'); if (error) { toast.error(`Customer directory failed: ${error.message}`); return } if (mounted) setCustomers(data || []) }; load(); return () => { mounted = false } }, [])
  const totals = useMemo(() => invoices.reduce((map, invoice) => { if (invoice.customer_id) { const current = map.get(invoice.customer_id) || { spent: 0, visits: 0 }; current.spent += Number(invoice.total || 0); current.visits += 1; map.set(invoice.customer_id, current) } return map }, new Map()), [invoices])
  const filtered = useMemo(() => customers.filter(customer => `${customer.full_name} ${customer.phone} ${customer.email || ''}`.toLowerCase().includes(debouncedQuery.toLowerCase())), [customers, debouncedQuery])
  const openNew = () => { setEditingId(null); setForm(emptyForm); setModalOpen(true) }
  const edit = customer => { setEditingId(customer.id); setForm({ full_name: customer.full_name || '', phone: customer.phone || '', email: customer.email || '' }); setModalOpen(true) }
  const save = async event => { event.preventDefault(); if (!form.full_name.trim() || !form.phone.trim()) return toast.error('Name and phone are required'); setBusy(true); let result; if (editingId) { const request = supabase.from('customers').update(form).eq('id', editingId); result = await request.select('*').single() } else { result = await supabase.from('customers').insert(form).select('*').single() } setBusy(false); if (result.error) return toast.error(`Customer save failed: ${result.error.message}`); setCustomers(current => editingId ? current.map(item => item.id === editingId ? result.data : item) : [result.data, ...current]); setModalOpen(false); toast.success('Customer saved') }
  return <><section className="page-intro"><div><p className="muted">Workspace customer directory and billing history.</p></div><button className="primary-btn" onClick={openNew}>Add customer</button></section><section className="panel invoice-page-panel"><div className="toolbar"><div className="search-box"><Icon name="search" size={17}/><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search name or phone…" autoFocus/></div><span className="muted">{filtered.length} customers</span></div><div className="table-scroll"><table><thead><tr><th>Name</th><th>Phone</th><th>Email</th><th>Total spent</th><th>Visits</th><th></th></tr></thead><tbody>{filtered.length ? filtered.map(customer => { const total = totals.get(customer.id) || { spent: Number(customer.total_spent || 0), visits: Number(customer.visit_count || 0) }; return <tr key={customer.id}><td data-label="Name"><strong>{customer.full_name}</strong></td><td data-label="Phone">{customer.phone}</td><td data-label="Email">{customer.email || '—'}</td><td data-label="Total spent"><strong>{money(total.spent)}</strong></td><td data-label="Visits">{total.visits}</td><td data-label="Edit"><button className="secondary-btn" onClick={() => edit(customer)}>Edit</button></td></tr> }) : <tr><td colSpan="6" className="empty-friendly compact">No customers found.</td></tr>}</tbody></table></div></section>{modalOpen && <div className="modal-backdrop"><section className="modal staff-modal"><div className="modal-head"><div><p className="eyebrow">CUSTOMER PROFILE</p><h3>{editingId ? 'Edit customer' : 'Add customer'}</h3></div><button className="close-btn" onClick={() => setModalOpen(false)}><Icon name="close"/></button></div><form className="staff-form" onSubmit={save}><label>Name<input value={form.full_name} onChange={event => setForm({ ...form, full_name: event.target.value })} required/></label><label>Phone number<input inputMode="tel" value={form.phone} onChange={event => setForm({ ...form, phone: event.target.value })} required/></label><label>Email<input type="email" value={form.email} onChange={event => setForm({ ...form, email: event.target.value })}/></label><div className="form-actions"><button type="button" className="secondary-btn" onClick={() => setModalOpen(false)}>Cancel</button><button className="primary-btn" disabled={busy}>{busy ? 'Saving…' : 'Save customer'}</button></div></form></section></div>}</>
}
