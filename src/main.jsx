...
function POS({ onInvoice, catalog, user, permissions, onOwnerOverride }) {
  ...
  const complete = async ({ payment, discount }) => {
    ...
    const saved = await onInvoice({ customer: 'Walk-in customer', items: cart, ...checkoutSummary, subtotal: discountedSubtotal, tax: discountedTax, total: finalTotal, discount: discountAmount, payment, status: 'Paid' })
    if (!saved) return
  ...
  return <>
    <section className="page-intro"><div><p className="muted">Quick Billing Counter <span className="live-dot">● Live</span></p><small className="muted">{user?.name} · USB/Bluetooth gun ready · scan a barcode ending with Enter</small></div><div className="low-stock-summary">{catalog.filter(item => item.stock != null && item.stock <= Number(item.min_stock_alert ?? 10)).length} low-stock alerts</div></section>
    ...
    {paymentOpen && <PaymentModal total={checkoutSummary?.total || 0} canApplyDiscounts={permissions.can_apply_discounts} onClose={() => setPaymentOpen(false)} onComplete={complete}/>} 
  </>
}

function PaymentModal({ total, canApplyDiscounts, onClose, onComplete }) { const [method, setMethod] = useState('UPI'); const [discount, setDiscount] = useState(''); const finalTotal = Math.max(0, Number(total) - (canApplyDiscounts ? Number(discount || 0) : 0)); const upiLink = `upi://pay?pa=billflow@upi&pn=BillFlow&am=${finalTotal}&cu=INR`; return <div className="modal-backdrop"><section className="modal payment-modal"><div className="modal-head"><div><p className="eyebrow">SECURE CHECKOUT</p><h3>Collect {money(finalTotal)}</h3></div><button className="close-btn" onClick={onClose}><Icon name="close"/></button></div><label className="discount-field">Custom discount<input type="number" min="0" max={total} step="0.01" value={discount} disabled={!canApplyDiscounts} onChange={event => setDiscount(event.target.value)} placeholder={canApplyDiscounts ? '0.00' : 'Owner permission required'}/>{!canApplyDiscounts && <small className="muted">Your role cannot apply checkout discounts.</small>}</label><div className="payment-methods">{['UPI', 'Cash', 'Card'].map(item => <button key={item} className={method === item ? 'selected' : ''} onClick={() => setMethod(item)}>{item}</button>)}</div>{method === 'UPI' && <div className="upi-panel"><div className="qr-placeholder">QR</div><p>Scan with any UPI app</p><a href={upiLink}>Open UPI payment</a></div>}<button className="primary-btn full" onClick={() => onComplete({ payment: method, discount: canApplyDiscounts ? discount : 0 })}><Icon name="check" size={16}/> Mark {method} paid</button></section></div> }
...
function StaffManagement({ staff, onCreateStaff, onUpdateStaff, onResetPin, onDeactivate }) {
  const emptyForm = { full_name: '', email: '', role: 'employee', access_pin: '' }
  const [form, setForm] = useState(emptyForm)
  ...
  const submitStaff = async event => { event.preventDefault(); if (!form.full_name.trim() || !form.email.trim() || form.access_pin.length < 4) return toast.error('Name, email, and a 4 digit PIN are required'); setBusy(true); const ok = await onCreateStaff(form); setBusy(false); if (ok) { setForm(emptyForm); setModal(null) } }
...
function RegisterLock({ user, onUnlock, onLogout }) { const [pin, setPin] = useState(''); const submit = event => { event.preventDefault(); if (pin === '1234') onUnlock(); else toast.error('Incorrect register PIN. Demo PIN is 1234.') }; return <div className="register-lock-overlay"><section className="modal register-lock-card"><div className="brand auth-brand"><div className="brand-mark">B</div><span>Bill<span>Flow</span></span></div><p className="eyebrow">REGISTER LOCKED</p><h2>Switch user or unlock register</h2><p className="muted">The browser session stays active while the counter is locked. In production, validate this PIN through the trusted staff function.</p><form onSubmit={submit}><label>Access PIN<input autoFocus inputMode="numeric" type="password" value={pin} onChange={event => setPin(event.target.value.replace(/\D/g, ''))} placeholder="Enter PIN" /></label><button className="primary-btn full">Unlock as {user.name}</button></form><button className="secondary-btn full" onClick={onLogout}>Sign out completely</button></section></div> }
...
function App() { const [user, setUser] = useState(() => supabase ? null : getDemoUser()); const [darkMode, setDarkMode] = useState(() => localStorage.getItem('billflow-theme') === 'dark'); const [catalog, setCatalog] = useState(() => supabase ? [] : products); const [invoices, setInvoices] = useState(() => { if (supabase) return []; try { return JSON.parse(localStorage.getItem('billflow-invoices')) || seedInvoices } catch { return seedInvoices } }); const [staff, setStaff] = useState([]); const [registerLocked, setRegisterLocked] = useState(false); const [overrideAction, setOverrideAction] = useState(null); useEffect(() => { localStorage.setItem('billflow-invoices', JSON.stringify(invoices)) }, [invoices]); useEffect(() => { localStorage.setItem('billflow-theme', darkMode ? 'dark' : 'light'); document.body.dataset.theme = darkMode ? 'dark' : 'light' }, [darkMode]); useEffect(() => { let mounted = true; if (supabase) { supabase.auth.getSession().then(async ({ data }) => { if (!mounted || !data.session?.user) return; const profile = await getProfile(data.session.user.id); if (mounted) setUser(mapAuthUser(data.session.user, profile)) }); const { data: listener } = supabase.auth.onAuthStateChange(async (_event, session) => { if (!session?.user) return setUser(null); const profile = await getProfile(session.user.id); if (mounted) setUser(mapAuthUser(session.user, profile)) }); return () => { mounted = false; listener.subscription.unsubscribe() } } return () => { mounted = false } }, []); useEffect(() => { if (!supabase || !user?.id) return; loadProducts(user.workspace_id).then(setCatalog); loadInvoices(user.workspace_id).then(setInvoices); if (supabase) supabase.from('profiles').update({ last_active_at: new Date().toISOString() }).eq('id', user.id).then(({ error }) => { if (error) console.warn('Last active update failed:', error.message) }); if (user.role === 'Owner') loadStaff(user.workspace_id).then(setStaff) }, [user?.id, user?.workspace_id, user?.role]); const updateCatalogStocks = updates => setCatalog(current => current.map(item => {
  ...
  return <BrowserRouter>{user ? <Layout user={user} darkMode={darkMode} onToggleTheme={() => setDarkMode(value => !value)} onLockRegister={() => setRegisterLocked(true)} onLogout={async () => { await supabase?.auth.signOut(); localStorage.removeItem('billflow-user'); setUser(null) }}><Routes><Route path="/" element={<Protected user={user} roles={['Owner']}><Overview invoices={invoices} catalog={catalog}/></Protected>}/><Route path="/pos" element={<Protected user={user} roles={['Owner', 'Employee']}><ErrorBoundary><POS onInvoice={createInvoice} catalog={catalog} user={user} permissions={user.permissions || profilePermissions(user)} onOwnerOverride={action => setOverrideAction(() => action)}/></ErrorBoundary></Protected>}/><Route path="/invoices" element={<Invoices invoices={invoices}/>}/><Route path="/invoice/:id" element={<InvoiceDetail invoices={invoices}/>}/><Route path="/receipt/:id" element={<InvoiceDetail invoices={invoices}/>}/><Route path="/inventory" element={<Protected user={user} roles={['Owner', 'Employee']}>{user.role === 'Owner' || user.permissions?.can_edit_inventory ? <Inventory catalog={catalog} onCreate={createProduct} onUpdate={updateProduct} onDelete={deleteProduct} onAdjustStock={adjustStock}/> : <SimplePage title="Inventory access restricted" text="Your owner can grant inventory editing permission from Settings." icon="barcode"/>}</Protected>}/><Route path="/customers" element={<Protected user={user} roles={['Owner']}><SimplePage title="Customer management" text="Owner-only customer records and billing history."/></Protected>}/><Route path="/reports" element={<Protected user={user} roles={['Owner']}><Reports invoices={invoices}/></Protected>}/><Route path="/settings" element={<Protected user={user} roles={['Owner']}><Settings user={user} staff={staff} onCreateStaff={createStaff} onUpdateStaff={updateStaff} onResetPin={resetStaffPin} onDeactivate={deactivateStaff}/></Protected>}/><Route path="*" element={<Navigate to="/invoices" replace/>}/></Routes>{registerLocked && <RegisterLock user={user} onUnlock={() => setRegisterLocked(false)} onLogout={async () => { await supabase?.auth.signOut(); localStorage.removeItem('billflow-user'); setUser(null) }}/>} {overrideAction && <OwnerOverrideModal onClose={() => setOverrideAction(null)} onConfirm={() => { const action = overrideAction; setOverrideAction(null); action?.() }}/>}</Layout> : <Routes><Route path="/invoice/:id" element={<InvoiceDetail invoices={invoices}/>}/><Route path="/receipt/:id" element={<InvoiceDetail invoices={invoices}/>}/><Route path="*" element={<Auth onAuth={nextUser => { localStorage.setItem('billflow-user', JSON.stringify(nextUser)); setUser(nextUser) }}/>}/></Routes>}<Toaster position="bottom-right"/></BrowserRouter> }
